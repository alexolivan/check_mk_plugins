...
def perfometer_shoutcastv2(row, check_command, perf_data):
    ...
    if peak_warn != 0 or peak_crit != 0:
        if peak_usage > int(perf_data[1][4]):
    	    color[3] = "#f00"
        elif peak_usage > int(perf_data[1][3]):
    	    color[3] = "#ff0"
        else:
    	    color[3] = "#0f0"
    else:
...
perfometers['check_mk-shoutcastv2'] = perfometer_shoutcastv2